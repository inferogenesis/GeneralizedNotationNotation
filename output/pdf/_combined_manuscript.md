...
This section reports what the project has actually produced and how each quantitative claim is grounded. Every number below is substituted at render time from the deterministic producer, which reads the repository state at commit 4832889, so each figure here is regenerated from the artifacts it describes rather than transcribed.

...
generated from the producer's token map at commit ae6035216. A
...
\protect\breaktt{src/gnn/STEP\_INDEX.md} at commit ae6035216; its footer
...
producer tokens at commit ae6035216, and the panel is generated by
...
model-family manifest at commit ae6035216, and its footer summarizes the
...
repository state at commit ae6035216, not typed by hand. The producer
...
state at commit ae6035216, so each figure here is regenerated from the
...
ae6035216.}\label{fig:family_matrix}
\end{figure}
...
ae6035216 --- the same token map that substitutes the prose counts, so
...
The test suite comprises 457 test files containing 4903 test functions,
exercising a source base of 683 Python files across 44 packages (198553
...
\breaktt{output/data/manuscript\_variables.json} (ae6035216): a code
change that alters, for example, the test inventory (457 test files,
4903 test functions) propagates into the prose on the next regeneration
...
into 44 Python packages spanning 683 source files and roughly 198553

...
generated from the producer's token map at commit 4832889. A plain-text
specification --- the \protect\texttt{StateSpaceBlock} declaring the
model's tensors, \protect\texttt{A}, \protect\texttt{B},
...
12, and 142 Model Context Protocol tools.}\label{fig:graphical_abstract}
\end{figure}
...
\item
  ...
  JAX, DisCoPy, PyTorch, NumPyro, cpomdp, Stan, bnlearn) that
  ...
  PyTorch, NumPyro, cpomdp, Stan, bnlearn) also execute at Step 12,
...
\breaktt{input/gnn\_files/discrete/} & PyMDP, RxInfer.jl,
...
NumPyro, cpomdp, Stan & RxInfer.jl, JAX, PyTorch, NumPyro, cpomdp,
Stan \\
Multi-agent & \texttt{nr\_agents} + per-agent matrix keys
(\texttt{A\_agent1}, \ldots) & \breaktt{input/gnn\_files/multiagent/} &
PyMDP, RxInfer.jl, ActiveInference.jl, JAX, DisCoPy, PyTorch, NumPyro,
Stan, bnlearn & PyMDP, RxInfer.jl, ActiveInference.jl, JAX, DisCoPy,
...
\protect\breaktt{src/gnn/STEP\_INDEX.md} at commit 4832889; its footer
...
producer tokens at commit 4832889, and the panel is generated by
...
PyMDP, RxInfer.jl, ActiveInference.jl, JAX, DisCoPy, PyTorch, NumPyro,
cpomdp, Stan, bnlearn --- so that a single GNN model can be instantiated
as a simulation in whichever computational ecosystem a researcher
already works in. Each backend is addressed through a registry key that
maps the abstract model onto that framework's idioms for representing
generative models and performing inference. The full mapping from
registry key to backend is given below.
...
model-family manifest at commit 4832889, and its footer summarizes the
...
ActiveInference.jl, JAX, DisCoPy, PyTorch, NumPyro, cpomdp, Stan,
bnlearn --- have an executor at this step, and execution is kind-aware
...
repository state at commit 4832889, not typed by hand. The producer
...
state at commit 4832889, so each figure here is regenerated from the
...
RxInfer.jl, JAX, PyTorch, NumPyro, cpomdp, Stan; reported unsupported
(not failed) on PyMDP, ActiveInference.jl, DisCoPy, bnlearn. \\
...
4832889.}\label{fig:family_matrix}
\end{figure}
...
4832889 --- the same token map that substitutes the prose counts, so the
figure cannot disagree with the text without failing the
...
The test suite comprises 456 test files containing 4900 test functions,
exercising a source base of 683 Python files across 44 packages (198544
...
\breaktt{output/data/manuscript\_variables.json} (4832889): a code change
that alters, for example, the test inventory (456 test files, 4900 test
functions) propagates into the prose on the next regeneration without
any manual editing.
...
into 44 Python packages spanning 683 source files and roughly 198544
...
NumPyro, cpomdp, Stan, bnlearn), of which the 10 listed as executable
(PyMDP, RxInfer.jl, ActiveInference.jl, JAX, DisCoPy, PyTorch, NumPyro,
cpomdp, Stan, bnlearn) also run at Step 12 --- with per-kind reach

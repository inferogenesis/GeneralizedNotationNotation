...
2. **The Output Target Frameworks**: How GNN compiles down to the ten render backends (PyMDP, RxInfer.jl, ActiveInference.jl, JAX, DisCoPy, PyTorch, NumPyro, cpomdp, Stan, bnlearn) registered in [`src/gnn/render/framework_registry.py`](../../src/gnn/render/framework_registry.py). Which code a backend emits depends on the **model kind** detected structurally from the declared parameterization keys — see [`gnn_syntax.md` § Parameterization families](gnn_syntax.md#parameterization-families).

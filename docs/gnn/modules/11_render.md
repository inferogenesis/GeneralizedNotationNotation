...
This module provides **POMDP-aware code generation** for GNN models. It translates parsed GNN/POMDP specifications into executable simulation code for ten frameworks: PyMDP, RxInfer.jl, ActiveInference.jl, JAX, DisCoPy, NumPyro and Stan are always rendered; PyTorch and bnlearn are also rendered but their runtimes are gated on manual installs (`available: False` in `src/gnn/render/framework_registry.py`); cpomdp renders continuous models only (its runtime is the `cpomdp` extra). Continuous (linear-Gaussian) models render on JAX, NumPyro, PyTorch, Stan, RxInfer.jl and cpomdp; PyMDP, ActiveInference.jl, DisCoPy and bnlearn report render status `unsupported` for them, and cpomdp reports discrete models `unsupported`.
...
#### cpomdp (Python)
- **Purpose**: Continuous active inference backend (exact Kalman filter plus exhaustive expected-free-energy search over a compass-rose action set)
- **Output**: Python scripts under `cpomdp/` (`<model>_cpomdp.py`) for continuous models; discrete models report `unsupported`
- **Notes**: Runtime is the optional `cpomdp` extra (`uv sync --extra cpomdp`); render options `control_mode` (`efe` | `parity`), `action_scale`, `horizon`, `goal_precision`
...
- **Notes**: Registered in `render.framework_registry` as POMDP-compatible with no required matrices; `render.health.check_renderers()` reports it alongside the other nine backends
...
- `target` (str): Target framework ("pymdp", "rxinfer", "activeinference_jl", "jax", "discopy", "bnlearn", "pytorch", "numpyro", "cpomdp", "stan")
...
- Framework artifacts are written under per-model/per-framework subfolders (POMDP-aware mode), typically one primary script per framework:
  ...
  - `cpomdp/<model_name>_cpomdp.py` (continuous models only)

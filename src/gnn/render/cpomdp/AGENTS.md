...
- The emitted script constructs, in order: `LinearGaussianModel` (`control = I_n` when a goal is declared, since GNN adds `u` directly to the state; `observation=CallableSensor(...)` when `R_x_family` is declared), `StateGoal(goal_mean)` under a fixed `R` or `ObservationGoal(H @ goal_mean)` under the callable sensor, a compass-rose `FiniteActionSet`, `EnumeratedEfeSearch` + `RecedingHorizonSelector`, and an `Agent` whose construction is cpomdp's goal/sensor pairing check.
- The sensor family library lives inside the emitted script (four jittable closures scaling the nominal `R`), never in GNN `src`; the vocabulary and arity table is `render.continuous_common.SENSOR_FAMILIES`. The simulation loop drives `KalmanBackend.infer_states` directly so it can record the full per-policy `G` vector each step. `EFESelector` is 1-D only and is not used.

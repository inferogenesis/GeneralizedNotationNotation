...
with the control input added directly to the state (cpomdp's
``control_matrix`` is the identity), and the prior placed on ``x_1`` (the first observation is folded
...
def resolve_render_options(options: Optional[Mapping[str, Any]]) -> Dict[str, Any]:
    """Apply defaults to the cpomdp render options and validate them.

    Keys other than the four below are ignored (the processor forwards its own).
    """
...
def sample_observation(model, x):
    C, R = model.observation_matrix, model.observation_noise
...
def build_planner(model, goal):
    """Exhaustive EFE search plus the observation-space preference it scores.

    The loop drives the search directly (not through a selector) because it
    records the full per-policy G vector each step.
    """
...
def run_simulation():
    ...
    if goal is not None:
        ...
        search = preference = evaluate = best_policy_terms = None
...
def generate_cpomdp_script(spec: ContinuousSpec, opts: Mapping[str, Any]) -> str:
    ...
    ``opts`` is the output of :func:`resolve_render_options`.
    """
